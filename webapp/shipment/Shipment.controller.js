 ...
 * @Description : This is Shipment view controller
 */
sap.ui.controller("mc.ccp.shipment.Shipment", {
    /**
...
//        var oDataModel = new sap.ui.model.odata.ODataModel(MCAPP.getServiceUrl(MCAPP.getComponent().getModel('service').getProperty('/ShipmentService')));
    	var oDataModel = new sap.ui.model.json.JSONModel();
        this.getView().data('oDataModel', oDataModel);
        var thisContext = this;
        oDataModel.attachRequestCompleted(thisContext._dataLoadSuccess,this);
        oDataModel.loadData("json/ShipmentService.json",{},false);
    	/*oDataModel.read('ZCCP_SHIP_HDR_SET', null, {
    		"$expand":"ZCCP_SHIP_HDR_TO_ITEM_NAV",
    		"$format":"json",
            "$filter":MCAPP.getFilterForShipmentDomesticScreen (),
            }, false, 
    ...
    onPressSave: function(oEvent) {
        ...
        if (changedRecords.length <= 0) {
        	...
        	this._removeFlagFromChangedRecords(changedRecords);
        	//var oDataModel = this.getView().data('oDataModel');
        	var oDataModel = new sap.ui.model.odata.ODataModel(MCAPP.getServiceUrl(MCAPP.getComponent().getModel('service').getProperty('/ShipmentSaveService')));
            var dummyHeader = {};
            dummyHeader.ZCCP_DUMMY_TO_SHIP_HDR_NAV = {};
            dummyHeader.ZCCP_DUMMY_TO_SHIP_HDR_NAV.results = changedRecords;
            oDataModel.create('/ZCCP_SHIP_HDR_HEAD_DUMMY_SET', dummyHeader, null, function(oData, response) {
    ...
    onAfterRendering: function() {
        ...
        MCAPP.updateHeaderShipment(this);
    ...
    /***
     * Event Handler for Expand All/ Collapse All option. If clicked event is Expand, get all the ShipmentItemTable controls from RowRepeater
     * and Call expand function on each row.
     * Else
     * get all the ShipmentItemTable controls from RowRepeater and call collapse function on each row.
     * @param evt
     */
    ...
    onPressCancel: function() {
        var that = this;
        var oModelData = that.getView().getModel().getData().results;
